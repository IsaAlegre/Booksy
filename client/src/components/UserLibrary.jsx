      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredBooks.map((book) => {
          ...
          return (
            <article
              key={book.id}
              className="bg-[#F9F9FB] rounded-2xl shadow-xl p-4 flex gap-4 transition-transform hover:-translate-y-1 hover:shadow-lg"
            >
            <figure className="w-24 sm:w-28 flex-shrink-0 md:h-auto">
              ...
              <div className="flex flex-col justify-start py-1 flex-grow">
                ...
                {/* Estado */}
                <span className="mb-2 inline-block px-3 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-700 self-start">
                  {
                    STATUS_OPTIONS.find(
                ...
                <Link
                    ...
                    className="mt-auto text-purple-600 hover:text-purple-800  text-xs font-medium transition-colors duration-200 self-start"
                    >
                    Ver detalles
                </Link>
              </div>
